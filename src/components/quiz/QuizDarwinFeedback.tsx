...
interface QuizDarwinFeedbackProps {
  isCorrect: boolean;
  explanation: string;
  points: number;
...
export const QuizDarwinFeedback = ({ 
  isCorrect, 
  explanation, 
  points, 
  affinityLevel,
  affinityProgress 
}: QuizDarwinFeedbackProps) => {
  return (
    <div className="w-full bg-secondary/30 rounded-lg p-4 space-y-4 animate-fade-in">
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="w-28 h-28 flex-shrink-0">
          <img 
            src={isCorrect ? "/lovable-uploads/darwin_acerto.png" : "/lovable-uploads/darwin_erro.png"}
            alt={`Darwin ${isCorrect ? 'feliz' : 'pensativo'}`}
        ...
        <div className="flex-1 bg-background border-2 border-primary/20 rounded-xl p-3 relative">
          ...
          <p className="text-sm font-medium text-foreground leading-relaxed">
            {isCorrect 
              ? "Uma seleção natural... de resposta correta! Você observou as evidências e chegou à conclusão mais adaptada."
              : "A evolução exige tentativas. Analise os detalhes e tente novamente. A adaptação virá."
            }
          </p>
        </div>
      </div>
      ...
      <div className="bg-primary/5 rounded-lg p-3 space-y-2">
        <div className="flex justify-between items-center">
          ...
          <span className="text-xs text-muted-foreground">
            Afinidade com Darwin: Nível {affinityLevel}
          </span>
        </div>
