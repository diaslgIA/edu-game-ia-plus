...
import DynamicQuizQuestion from './quiz/DynamicQuizQuestion';
...
interface DynamicSubjectQuizProps {
  subject: string;
  topic?: string;
...
const DynamicSubjectQuiz: React.FC<DynamicSubjectQuizProps> = ({ 
  subject, 
  topic, 
  onComplete, 
  onBack 
}) => {
  const { questions, loading, error } = useQuizQuestions(subject);
  ...
  useEffect(() => {
    if (questions.length > 0) {
      ...
      // Converter para formato compatível com o componente existente
      const formattedQuestions = selectedQuestions.map(q => {
        ...
        return {
          ...
          difficulty: getDifficultyLabel(q.difficulty_level),
  ...
  const getDifficultyLabel = (difficulty: string) => {
    ...
    return difficultyMap[difficulty?.toLowerCase()] || 'Médio';
  };
...
export default DynamicSubjectQuiz;
